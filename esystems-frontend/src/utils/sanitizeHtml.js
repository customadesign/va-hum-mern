 ...
 * - Blocks javascript:, vbscript:, data: (except none), on* handlers, style, and unknown tags/attrs
...
export function sanitizeHtml(input) {
  ...
  try {
    if (typeof window !== 'undefined' && window.DOMPurify && typeof window.DOMPurify.sanitize === 'function') {
      return window.DOMPurify.sanitize(html, {
        ...
        // http, https, mailto, tel, or site-relative URLs starting with a single /
  ...
  try {
    ...
    const isSafeHref = (href) => {
      ...
      if (href.startsWith('/')) return true; // site-relative
    ...
    const walk = (root) => {
      ...
      nodeList.forEach((el) => {
        ...
        if (!allowedTags.has(tag)) {
          // Replace disallowed elements with text content
        ...
        Array.from(el.attributes).forEach((attr) => {
          ...
          if (name.startsWith('on')) el.removeAttribute(name); // remove event handlers
        ...
        if (tag === 'A') {
          ...
          // Internal links open in same tab; external open in new tab
          a.setAttribute('target', safeHref.startsWith('/') ? '_self' : '_blank');
          a.setAttribute('rel', 'noopener noreferrer');

          // Keep only allowlisted attributes for <a>
          ...
          // For non-anchor allowed tags: remove all attributes except class
          Array.from(el.attributes).forEach((attr) => {
    ...
    // If anything goes wrong, strip tags
    return html.replace(/<[^>]*>/g, '');
  }
}
