...
export default function VAProfile() {
  ...
  // Fetch current VA profile with better caching to prevent excessive requests
  const { data: profile, isLoading } = useQuery("vaProfile", async () => {
    ...
    onError: (error) => {
      console.error('VA Profile fetch error:', error);
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      // Update the local cache directly instead of invalidating to prevent excessive refetches
      queryClient.setQueryData("vaProfile", (oldData) => {
        if (oldData?.data) {
          return {
            ...oldData,
            data: {
      ...
      // Also update user profile cache
      ...
      if (error.response?.status === 413) {
        toast.error("File size too large. Please choose an image under 10MB.");
      } else if (error.response?.status === 415) {
        toast.error("Unsupported file type. Please use JPG, PNG, or WebP format.");
      }
    } finally {
  ...
  const handleVideoChange = async (e) => {
    ...
    try {
      ...
      // Provide more specific error guidance
      if (error.response?.status === 413) {
        toast.error("Video file size too large. Please choose a video under 1GB.");
      } else if (error.response?.status === 415) {
        toast.error("Unsupported video format. Please use MP4, WebM, or MOV format.");
      } else if (error.response?.status === 408) {
        toast.error("Upload timeout. Please try a smaller video or check your connection.");
      }
    } finally {
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col space-y-6">
          ...
          <form onSubmit={formik.handleSubmit} className="space-y-8">
            ...
            {/* Submit Button */}
            <div className="flex justify-end mt-8 mb-24 pb-16 mr-4 sm:mr-6 lg:mr-0">
              <button
                type="submit"
