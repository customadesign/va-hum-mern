  ...
  const profileCompletion = useMemo(() => {
    ...
    const requiredFields = [
      ...
      { field: "hero", weight: 10, check: () => profile.hero?.trim() },
      {
        field: "bio",
        weight: 15,
        check: () => profile.bio?.trim() && profile.bio.length >= 50,
      },
      {
        field: "location",
        weight: 10,
      ...
      { field: "email", weight: 10, check: () => profile.email?.trim() },
      {
        field: "specialties",
        weight: 15,
        check: () => profile.specialtyIds?.length > 0,
      },
      {
        field: "roleType",
        weight: 5,
        check: () => Object.values(profile.roleType || {}).some(Boolean),
      },
      {
        field: "roleLevel",
        weight: 5,
        check: () => Object.values(profile.roleLevel || {}).some(Boolean),
      },
      ...
      // Enhanced fields (medium weight)
      {
        ...
        weight: 10,
        check: () =>
          profile.preferredMinHourlyRate && profile.preferredMaxHourlyRate,
      },
      { field: "phone", weight: 5, check: () => profile.phone?.trim() },
      {
        field: "onlinePresence",
        weight: 5,
        check: () => profile.website?.trim() || profile.linkedin?.trim(),
      },
      {
        field: "discAssessment",
        weight: 10,
        check: () => profile.discPrimaryType,
      },
    ];
