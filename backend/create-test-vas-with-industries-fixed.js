  ...
  try {
    ...
    const testVAs = [
      {
        ...
        industry: 'digital_marketing', // Use valid enum value
        yearsOfExperience: 5,
    ...
    for (const vaData of testVAs) {
      try {
        ...
        // Create User first (required by VA model)
        let user = await User.findOne({ email: vaData.email });
        if (!user) {
          ...
          console.log(`✅ Created User: ${vaData.email}`);
        }

        // Create VA with user reference
        ...
        console.error(`❌ Failed to create VA ${vaData.email}:`, error.message);
      }
    }
