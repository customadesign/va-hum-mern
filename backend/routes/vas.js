...
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    // Enhanced search - prioritize exact name matches, then prefix, then contains
    if (search) {
      // Clean search term
      const cleanSearch = search.trim();
      
      query.$or = [
        // Priority 1: Exact name match (highest relevance)
        { name: { $regex: `^${cleanSearch}$`, $options: 'i' } },
        // Priority 2: Prefix name match (high relevance)
        { name: { $regex: `^${cleanSearch}`, $options: 'i' } },
        // Priority 3: Contains name match (medium relevance)
        { name: { $regex: cleanSearch, $options: 'i' } },
        // Priority 4: Bio contains (lower relevance)
        { bio: { $regex: cleanSearch, $options: 'i' } },
        // Priority 5: Hero contains (lower relevance)
        { hero: { $regex: cleanSearch, $options: 'i' } },
        // Priority 6: Skills contain (lower relevance)
        { skills: { $regex: cleanSearch, $options: 'i' } },
        // Priority 7: Specialties contain (lower relevance)
        { 'specialties.name': { $regex: cleanSearch, $options: 'i' } },
        // Priority 8: Location contains (lowest relevance)
        { 'location.city': { $regex: cleanSearch, $options: 'i' } },
        { 'location.state': { $regex: cleanSearch, $options: 'i' } },
        { 'location.country': { $regex: cleanSearch, $options: 'i' } }
      ];
    }
    ...
    try {
      // Enhanced sorting for search relevance
      let sortOptions = sort;
      if (search) {
        // Simple enhanced sorting: prioritize exact matches first
        sortOptions = {
          // First by searchStatus (actively_looking first)
          searchStatus: -1,
...
// @access  Public
router.get('/test-upload', optionalAuth, async (req, res) => {
  try {
...
// @access  Private (VA only)
router.post('/me/upload', protect, handleUnifiedUpload('image', 'profile-images'), async (req, res) => {
  console.log('POST /api/vas/me/upload - Upload request received');
...
// @access  Private (VA only)
router.post('/me/upload-video', protect, handleUnifiedUpload('video', 'va-videos'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
