...
const maxRequests = parseInt(process.env.RATE_LIMIT_MAX || '5000', 10); // Increased to 5000 for development
...
const limiter = rateLimit({
  ...
  max: maxRequests,
  skip: (req) => {
    // Skip rate limiting for all endpoints in development to prevent issues
    if (process.env.NODE_ENV !== 'production') {
      return true;
    }
    return false;
  ...
  handler: (req, res) => {
    console.log('Rate limit exceeded for:', {
      ip: req.ip,
      path: req.path,
    ...
    res.status(429).json({
      ...
      retryAfter: Math.round(windowMinutes * 60 / 2) // Suggest half the window time
    });
  }
});
app.use('/api/', limiter);
